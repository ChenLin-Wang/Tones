...
export const freqs = (tones: string[]): number[][] => {
    ...
    var semiTones: { flap: boolean, tone: number }[] = []
    for (const t of tones) {
        const { flap, key } = toneShift(t.slice(0, -1))
        if (basicKeys.includes(allKeys[key] as keyof BasicInotonationSequence)) {
            ...
            res.push(ts)
        } else semiTones.push({ flap: flap, tone: key })
    }

    var ts: number[] = []
    for (const inoName of inotonations) {
        for (const { flap, tone: s } of semiTones) {
            var fitGap: number | null = null
            var fitTone: Key | null = null
            var gapI: number | null = null
            const ints = inotons[inoName].intervals
            for (const b of basicTones) {
                const gap = s - b
                for (var istr in (ints.seqs as number[])) {
                    const i = parseInt(istr)
                    const curI = ints.seqs[i]
            ...
            if (fitGap && fitTone && inotons[inoName].basicFreqs) {
                ts.push(inotons[inoName].basicFreqs[allKeys[fitTone] as keyof BasicFreqs] * (fitGap > 0 ? ints.ins[fitGap] : 1 / ints.ins[-fitGap]) / (flap ? 2 : 1))
            } else {
                ts.push(1)
            }
        }
...
const toneShift = (tone: string): { flap: boolean, key: Key } => {
    ...
    return gap >= 0 ? { flap: false, key: gap } : { flap: true, key: 11 + Key.A }
}
